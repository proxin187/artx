...
drawstatusbar(BarArg *a, char* stext)
{
	...
	// Add vertical inset for status bar only
	int status_inset = 5; // 5px inset top and bottom
	y += status_inset;
	int bar_height = a->h - (status_inset * 2);
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTWM(text) - lrpad;
			drw_text(drw, x, y, w, bar_height, 0, text, 0, True);
